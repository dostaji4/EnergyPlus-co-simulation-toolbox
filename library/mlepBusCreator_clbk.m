function mlepBusCreator_clbk(block, type)
%MLEPBUSCREATOR_CLBK - Callback functions for the 'mlepBusCreator' block.
% Valid type options are 'popup', 'button'.
...
switch type
    case 'popup'
        mlepBusCreator_popup(block)
    case 'button'
        mlepBusCreator_button(block)
    otherwise
        error('Unknown callback: ''%s.''', type);        
end
end

function mlepBusCreator_popup(block)
...
function mlepBusCreator_button(block)
% Using variable names terminated with "_BOBC" to lessen the chances of
% collisions with existing workspace variables.
